...
class GenDataset(Dataset):
    def __init__(self, data, max_limit = 25000):
        data = np.load(data, allow_pickle = True)
        self.snps = data["snps"][:,:max_limit].astype(float)
...
def loader(DATA_PATH = os.path.join(os.environ.get('USER_PATH'), 'data/ancestry_datasets'), batch_size = 10):
    dataset = GenDataset(data = f'{DATA_PATH}/All_chm_World/all_chm_combined_snps_world_2M_with_labels.npz')
